...
def save_to_excel(image_name, anp_results, iqa_score, nima_mean, nima_std, excel_path=save_path):
    iqa_keys = ["quality", "brightness", "noisiness", "colorfullness", "sharpness"]
    nima_keys = ["NIMA mean", "NIMA std"]

    row_data = [image_name]
    for anp, score in anp_results:
        row_data.extend([anp, round(float(score), 4)])
    ...
    if os.path.exists(excel_path):
        ...
        sheet = wb.active
        header = ["Image"]
        for i in range(len(anp_results)):
            header.extend([f"ANP {i+1}", f"Score {i+1}"])
        header.extend(iqa_keys)
        header.extend(nima_keys)
        sheet.append(header)

    current_header = [cell.value for cell in sheet[1]]
    expected_anp_fields = len(anp_results) * 2
    current_anp_fields = len(current_header) - len(iqa_keys) - 1
    if expected_anp_fields > current_anp_fields:
        for i in range(current_anp_fields // 2 + 1, len(anp_results) + 1):
            sheet.cell(row=1, column=2 + (i - 1) * 2, value=f"ANP {i}")
            sheet.cell(row=1, column=3 + (i - 1) * 2, value=f"Score {i}")
